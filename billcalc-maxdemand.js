...
function simpleBasicChargeCal(tariffComps, kva, kwh) {
    ...
    for (var i = 0; i < valBasicChargeHigh.length; i++) {
        ...
        if (block == 0) {
            ...
            remainUnits -= partialUnits;
        } else if (remainUnits <= counter) {
            partialUnits = remainUnits;
            remainUnits -= remainUnits;
        } else if (remainUnits > counter) {
            paritalUnits = counter;
            remainUnits -= counter;
        }
...
function demandChargeCalculator(hvTariffComps, lvTariffComps, totalConsumed) {

    var valDemandChargeHigh = hvTariffComps;
    var valDemandChargeLow = lvTariffComps;
